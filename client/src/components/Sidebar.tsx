...
interface SidebarProps { categories: Category[]; selectedCategoryId: string | null; onSelectCategory: (c: string | null, s?: string | null) => void; collapsed: boolean; onToggleCollapse: () => void; mobileOpen: boolean; onMobileClose: () => void; }

export default function Sidebar({ categories, selectedCategoryId, onSelectCategory, collapsed, onToggleCollapse, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      <div className="flex items-center h-[72px] px-6 shrink-0 cursor-pointer hover:opacity-80" onClick={() => { if (location !== "/") setLocation("/"); onSelectCategory(null, null); if (mobileOpen) onMobileClose(); }}>
        {collapsed ? (
