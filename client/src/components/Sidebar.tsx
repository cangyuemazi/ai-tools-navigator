...
export default function Sidebar({ categories, selectedCategoryId, onSelectCategory, collapsed, onToggleCollapse, mobileOpen, onMobileClose, activeSectionId, onScrollToCategory, onNavigateHome, onNavigateAllTools }: SidebarProps) {
  ...
  const handleCatClick = (id: string, hasC: boolean) => { if (hasC && !collapsed) toggleExpand(id); if (onScrollToCategory && location === "/") { onScrollToCategory(id); onSelectCategory(id, null); return; } if (location !== "/") setLocation(`/?category=${id}`); onSelectCategory(id, null); };
  const handleSubClick = (cId: string, sId: string) => { if (onScrollToCategory && location === "/") { onScrollToCategory(cId, sId); onSelectCategory(cId, sId); return; } if (location !== "/") setLocation(`/?category=${cId}&sub=${sId}`); onSelectCategory(cId, sId); };
