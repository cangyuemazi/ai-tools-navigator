...
export default function Admin() {
  ...
  const [siteSettings, setSiteSettings] = useState({ name: "", logo: "", favicon: "", titleFontSize: 17, backgroundColor: "#f5f5f7", companyIntro: "", icp: "", email: "" });
          ...
          <div className="bg-white p-8 rounded-[20px] shadow-sm border border-[#e8e8ed] animate-in fade-in">
            ...
            <div className="grid md:grid-cols-2 gap-10">
              <div className="space-y-6">
                ...
                <div><label className="block text-sm font-medium text-[#6e6e73] mb-2">侧边栏网站名称字体大小 (默认: 17)</label><input type="number" value={siteSettings.titleFontSize} onChange={e => setSiteSettings({...siteSettings, titleFontSize: parseInt(e.target.value)})} className="w-full px-4 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[10px] outline-none" /></div>
                <div><label className="block text-sm font-medium text-[#6e6e73] mb-2">网站全局背景颜色 (默认: #f5f5f7)</label><input type="text" value={siteSettings.backgroundColor} onChange={e => setSiteSettings({...siteSettings, backgroundColor: e.target.value})} className="w-full px-4 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[10px] outline-none" /></div>
                ...
                <div><label className="block text-sm font-medium text-[#6e6e73] mb-2">浏览器标签页图标 (Favicon .ico/.png)</label><div className="flex items-center gap-4"><div className="w-14 h-14 rounded-[12px] bg-[#f5f5f7] border flex items-center justify-center p-1">{siteSettings.favicon ? <img src={siteSettings.favicon} className="w-full h-full object-contain" /> : <span className="text-xs text-[#86868b]">无</span>}</div><label className="px-4 py-2 bg-white border border-[#d2d2d7] rounded-[8px] text-sm cursor-pointer hover:bg-[#f5f5f7]">{uploading ? "中..." : "上传Favicon"}<input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, url => setSiteSettings({...siteSettings, favicon: url}))} className="hidden" /></label></div></div>
              </div>
              <div className="space-y-6">
                <div className="p-5 bg-[#f5f5f7] rounded-[16px] border border-[#e8e8ed]">
                  ...
                  <div className="space-y-4">
                    ...
                    <div><label className="block text-sm font-medium text-[#6e6e73] mb-2">工信部备案号 (ICP)</label><input type="text" value={siteSettings.icp} onChange={e => setSiteSettings({...siteSettings, icp: e.target.value})} className="w-full px-4 py-2.5 bg-white border border-[#d2d2d7] rounded-[10px] outline-none" placeholder="京ICP备12345678号" /></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-[24px] w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2"><label className="block text-sm mb-1 text-[#6e6e73] font-medium">工具名称 *</label><input type="text" value={toolForm.name} onChange={e => setToolForm({...toolForm, name: e.target.value})} className="w-full px-3 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[8px] outline-none" /></div>
                {/* 👈 核心：手动排位分 */}
                <div><label className="block text-sm mb-1 text-[#0071e3] font-bold">排位分 (越高越前)</label><input type="number" value={toolForm.order} onChange={e => setToolForm({...toolForm, order: parseInt(e.target.value) || 0})} className="w-full px-3 py-2.5 bg-[#e6f0fa] border border-[#0071e3]/30 text-[#0071e3] font-bold rounded-[8px] outline-none" placeholder="0" /></div>
              </div>
              <div><label className="block text-sm mb-1 text-[#6e6e73] font-medium">链接 URL *</label><input type="text" value={toolForm.url} onChange={e => setToolForm({...toolForm, url: e.target.value})} className="w-full px-3 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[8px] outline-none" /></div>
              
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm mb-1 text-[#6e6e73] font-medium">主分类 *</label><select value={toolForm.categoryId} onChange={e => setToolForm({...toolForm, categoryId: e.target.value, subCategoryId: ""})} className="w-full px-3 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[8px] outline-none">{categories.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}</select></div>
                {selectedCategoryObj?.children?.length > 0 && (<div><label className="block text-sm mb-1 text-[#6e6e73] font-medium">子分类</label><select value={toolForm.subCategoryId} onChange={e => setToolForm({...toolForm, subCategoryId: e.target.value})} className="w-full px-3 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[8px] outline-none"><option value="">(无)</option>{selectedCategoryObj.children.map((sub: any) => <option key={sub.id} value={sub.id}>{sub.name}</option>)}</select></div>)}
              </div>

              <div><label className="block text-sm mb-1 text-[#6e6e73] font-medium">一句话简介</label><textarea value={toolForm.description} onChange={e => setToolForm({...toolForm, description: e.target.value})} className="w-full px-3 py-2.5 bg-[#f5f5f7] border border-[#d2d2d7] rounded-[8px] outline-none" /></div>
              <div><label className="block text-sm mb-1 text-[#6e6e73] font-medium">Logo 图片</label><div className="flex gap-4 items-center"><div className="w-12 h-12 bg-[#f5f5f7] border rounded-[10px] flex items-center justify-center">{toolForm.logo ? <img src={toolForm.logo} className="w-full h-full object-contain p-1" /> : <span className="text-xs">无</span>}</div><input type="text" value={toolForm.logo} onChange={e => setToolForm({...toolForm, logo: e.target.value})} className="flex-1 px-3 py-2.5 bg-[#f5f5f7] border rounded-[8px]" /><label className="px-4 py-2.5 bg-white border rounded-[8px] cursor-pointer text-sm">{uploading ? "中..." : "上传"}<input type="file" accept="image/*" onChange={e => handleFileUpload(e, url => setToolForm({...toolForm, logo: url}))} className="hidden"/></label></div></div>
              <div><label className="block text-sm mb-1 text-[#6e6e73] font-medium">标签 (逗号分隔)</label><input type="text" value={toolForm.tags} onChange={e => setToolForm({...toolForm, tags: e.target.value})} className="w-full px-3 py-2.5 bg-[#f5f5f7] border rounded-[8px]" /></div>
              <div className="p-4 bg-[#0071e3]/[0.03] border border-[#0071e3]/20 rounded-[12px]"><label className="flex items-center gap-2 font-semibold cursor-pointer"><input type="checkbox" checked={toolForm.isSponsored} onChange={e => setToolForm({...toolForm, isSponsored: e.target.checked})} className="w-4 h-4 accent-[#0071e3]" /> ⭐ 设为付费赞助优先展示</label>{toolForm.isSponsored && <input type="date" value={toolForm.sponsorExpiry} onChange={e => setToolForm({...toolForm, sponsorExpiry: e.target.value})} className="mt-3 px-3 py-1.5 border rounded-[6px] text-sm" />}</div>
            </div>
            <div className="mt-8 flex justify-end gap-3"><button onClick={() => setIsToolModalOpen(false)} className="px-5 py-2.5 bg-[#f5f5f7] rounded-[10px]">取消</button><button onClick={handleSaveTool} className="px-5 py-2.5 bg-[#0071e3] text-white rounded-[10px]">保存发布</button></div>
          </div>
        </div>
      )}
    </div>
